...
> **从未开发官方账号登录模式的汉化**，开发只进行到第三方登录为止。
> 用官方账号登录时界面会保持英文 —— 需要那一模式请移步
...
- [官方账号登录模式：本项目从未开发](#官方账号登录模式本项目从未开发)
...
| **安装中文** | `一键安装中文.bat` |
| 还原成英文/原样 | `一键还原.bat` |
| 只看看现在什么状态（安全，不改文件） | `检查状态.bat` |

安装时会弹一次 **UAC 确认框，点「是」**（原因见下方「为什么一定要管理员权限」）。
安装过程在一个管理员窗口里进行，带步骤编号和进度条；**请不要中途关掉那个窗口**。
...
只看窗口右上角的 × 是**不够**的，Claude 会留在托盘里继续跑，界面不会刷新。
...
1. **会修改 Claude 的安装文件**
   ...
   不碰 `claude.exe`，不改数字签名，不动 `app.asar`。
...
4. **不影响账号安全**
   ...
   因为不碰 `app.asar`、不改 `Claude.exe`，**数字签名保持完好**，
...
**只适用于第三方登录状态。** 官方模式的汉化从未开发（原因见下一节），
本项目的能力范围到第三方登录为止。
...
> **一句话结论**：**本项目只做第三方登录状态的汉化，官方账号登录模式从未开发。**
> 如果你是用官方账号登录 Claude Desktop，这一版**不会**把界面变中文，
> 而且我们**没有在做**这件事。想让官方登录也变中文，请移步
> [javaht/claude-desktop-zh-cn](https://github.com/javaht/claude-desktop-zh-cn)。
...
| | 状态 |
|---|---|
| 第三方登录（用 CC Switch 之类路由第三方模型） | ✅ **本项目支持**，功能完整 |
| 官方账号登录 | ❌ **不在本项目范围内**，界面保持英文 |

下面几节解释**为什么官方模式一开始就没做** ——
包括原理上的障碍、以及要做的话必须付出什么代价。
写这些不是"预告"，而是**说明这个边界是刻意划的，不是遗漏**。

### 现象：为什么补了语言文件，官方登录下还是英文

本项目的原理是**补本地语言文件**：往 Claude 安装目录里放 `zh-CN.json`，
再把 `zh-CN` 注册进前端的语言白名单，最后把 `config.json` 的界面语言设成 `zh-CN`。

这套做法在**第三方 API 模式**下完全有效，因为那种模式下界面是本地渲染的。

但**官方账号登录**时，界面渲染的是**远程 `https://claude.ai` 的网页**，
...
**这直接决定了：官方模式无法用"补语言文件"这条路解决。** 本项目的方法论在这里失效。

### 官方模式需要什么方案（我们调研过，但没有做）

既然远程页面的文字不来自本地资源，唯一可行的办法是**在运行时把翻译脚本注入页面**，
像浏览器插件那样替换界面文本。
...
**但这条路本项目没有走。** 原因是它必须突破下面这道关卡，而代价我们不愿让用户承担。
...
要过这一关，就必须**同时改写 `Claude.exe` 里内嵌的哈希值**，
于是必然产生三个后果：
...
**所以本项目的立场是**：不做任何会改动 `app.asar`、会破坏 `Claude.exe` 签名的事。
这也是为什么「它做了什么、没做什么」那一节明确写着**不碰 `claude.exe` / `app.asar`**。
...
2. **官方账号登录** → 移步
   ...
   README 里 —— 请自行判断是否接受。
3. **本项目不会转向官方模式**。如果你希望这里支持，欢迎提 Issue 讨论，
   但请先理解上面那三个后果。
