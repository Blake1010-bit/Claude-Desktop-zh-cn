...
/** WindowsApps 下 MSIX 版 Claude 的常见版本号（用于在无法列目录时逐个试探）。 */
const MSIX_VERSION_PROBES = [
  // 精确到当前主流的几个版本；探测成本极低（只是一次 existsSync）
  '1.52386.3.0',
  '1.40609.0',
]
...
function msixCandidates() {
  ...
  for (const v of MSIX_VERSION_PROBES) {
    for (const s of MSIX_SUFFIXES) {
      out.push(`Claude_${v}${s}`)
    }
  }
  // 再兼容性兜底：如果父目录碰巧能列，就用实际存在的名字
  for (const name of safeReadDir('C:\\Program Files\\WindowsApps')) {
    if (/^Claude_/i.test(name)) out.push(name)
  }
  return out
}
...
function candidates() {
  ...
  if (WIN) {
    ...
    for (const name of msixCandidates()) {
